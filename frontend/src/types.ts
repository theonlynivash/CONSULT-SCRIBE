...
export function normalizeDraft(raw: Partial<ReportDraft> | null | undefined): ReportDraft | null {
  ...
  return {
    ...
    patientProblems: (raw.patientProblems ?? []).map((item: any) => {
      ...
      // Some older/LLM drafts returned duration and severity in the opposite
      // fields. Correct that shape once at the boundary so the review UI and
      // PDF always display the attributes under the right labels.
