  ...
  useEffect(() => {
    ...
    function renderButton() {
      ...
      window.google.accounts.id.renderButton(containerRef.current, {
        ...
        text: 'continue_with',
        logo_alignment: 'left',
        width: Math.round(containerRef.current.offsetWidth) || 320,
      });
    }
  ...
  return (
    <>
      ...
      <div className="clay-social-row">
        <div ref={containerRef} className="clay-google-mount" style={googleUnavailable ? { display: 'none' } : undefined} />
        {googleUnavailable && (
          <button type="button" className="clay-social-btn clay-social-google" disabled title="Google sign-in is not configured">
            <GoogleIcon />
            Google
          </button>
        )}
        <button type="button" className="clay-social-btn clay-social-apple" disabled title="Coming soon">
          <AppleIcon />
          Apple
        </button>
      </div>
